...
import (
	...
	"github.com/popcateum/go-popcateum/accounts/usbwallet/trezor"
...
// or whether the Popcateum app was not started on it.
...
// Popcateum address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.PopcateumAddress)
	if _, err := w.trezorExchange(&trezor.PopcateumGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.PopcateumSignTx{
	...
	response := new(trezor.PopcateumTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.PopcateumTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
