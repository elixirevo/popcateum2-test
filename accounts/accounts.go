...
// Package accounts implements high level Popcateum account management.
package accounts
...
// Account represents an Popcateum account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // Popcateum account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain popcateum.ChainStateReader)
	...
	// by the Popcateum prefix scheme
	// It looks up the account specified either solely via its address contained within,
	// or optionally with the aid of any location metadata from the embedded URL field.
	//
	// If the wallet requires additional authentication to sign the request (e.g.
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19Popcateum Signed Message:\n%d%s", len(data), string(data))
