...
// TestAttachWithHeaders tests that 'gpop attach' with custom headers works, i.e
...
// TestAttachWithHeaders tests that 'gpop db --remotedb' with custom headers works, i.e
// that custom headers are forwarded to the target.
...
func testReceiveHeaders(t *testing.T, ln net.Listener, gpopArgs ...string) {
	var ok uint32
	server := &http.Server{
		Addr: "localhost:0",
