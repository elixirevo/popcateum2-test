...
// gpop is the official command-line client for Popcateum.
package main
...
import (
	...
	_ "github.com/popcateum/go-popcateum/eth/tracers/js"
	_ "github.com/popcateum/go-popcateum/eth/tracers/native"

	"github.com/urfave/cli/v2"
)

const (
	clientIdentifier = "gpop" // Client identifier to advertise over the network
)
...
var app = flags.NewApp("the go-popcateum command line interface")

func init() {
	// Initialize the CLI app and start Gpop
	app.Action = gpop
	app.Copyright = "Copyright 2013-2023 The go-popcateum Authors"
...
func prepare(ctx *cli.Context) {
	...
	case ctx.IsSet(utils.RinkebyFlag.Name):
		log.Info("Starting Gpop on Rinkeby testnet...")

	case ctx.IsSet(utils.GoerliFlag.Name):
		log.Info("Starting Gpop on Görli testnet...")

	case ctx.IsSet(utils.SepoliaFlag.Name):
		log.Info("Starting Gpop on Sepolia testnet...")

	case ctx.IsSet(utils.DeveloperFlag.Name):
		log.Info("Starting Gpop in ephemeral dev mode...")
		log.Warn(`You are running Gpop in --dev mode. Please note the following:
...
`)
	...
	case !ctx.IsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting Gpop on Popcateum mainnet...")
	}
...
// gpop is the main entry point into the system if no special subcommand is run.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func gpop(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node, backend ethapi.Backend, isConsole bool) {
	...
	// Create a client to interact with local gpop node.
	...
	if ctx.Bool(utils.MiningEnabledFlag.Name) || ctx.Bool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full Popcateum node is running
		if ctx.String(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		ethBackend, ok := backend.(*eth.EthAPIBackend)
		if !ok {
			utils.Fatalf("Popcateum service not running")
		}
