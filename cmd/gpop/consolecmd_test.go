...
func TestConsoleWelcome(t *testing.T) {
	...
	gpop := runMinimalGpop(t, "--miner.etherbase", coinbase, "console")
...
func TestAttachWelcome(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		...
		ipc = filepath.Join(t.TempDir(), "gpop.ipc")
	}
	// And HTTP + WS attachment
	p := trulyRandInt(1024, 65533) // Yeah, sometimes this will fail, sorry :P
	httpPort = strconv.Itoa(p)
	wsPort = strconv.Itoa(p + 1)
	gpop := runMinimalGpop(t, "--miner.etherbase", "0x8605cdbbdb6d264aa742e77020dcbc58fcdce182",
		"--ipcpath", ipc,
...
func testAttachWelcome(t *testing.T, gpop *testgpop, endpoint, apis string) {
	// Attach to a running gpop node and terminate immediately
	...
	attach.SetTemplateFunc("etherbase", func() string { return gpop.Etherbase })
