...
Use `devp2p enrdump <base64>` to verify and display an Popcateum Node Record.
...
Run `devp2p key to-enode mynode.key -ip 127.0.0.1 -tcp 60606` to create an enode:// URL
...
1. initialize the gpop node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run gpop with the following flags:
```
gpop --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the gpop node:
 ```
...
To run the eth66 protocol test suite, initialize a gpop node as described above and run the following command,
replacing `<enode>` with the enode of the gpop node:

 ```
 devp2p rlpx eth66-test <enode> cmd/devp2p/internal/ethtest/testdata/chain.rlp cmd/devp2p/internal/ethtest/testdata/genesis.json
```

[eth]: https://github.com/popcateum/devp2p/blob/master/caps/eth.md
[dns-tutorial]: https://gpop.ethereum.org/docs/developers/gpop-developer/dns-discovery-setup
[discv4]: https://github.com/popcateum/devp2p/tree/master/discv4.md
[discv5]: https://github.com/popcateum/devp2p/tree/master/discv5/discv5.md
