...
// Package gpopclient provides an RPC client for gpop-specific APIs.
package gpopclient
...
// Client is a wrapper around rpc.Client that implements gpop-specific functionality.
//
// If you want to use the standardized Popcateum RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg popcateum.CallMsg) (*types.AccessList, uint64, string, error) {
...
func (ec *Client) CallContract(ctx context.Context, msg popcateum.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
...
// GCStats retrieves the current garbage collection stats from a gpop node.
...
// MemStats retrieves the current memory stats from a gpop node.
...
// GetNodeInfo retrieves the node info of a gpop node.
