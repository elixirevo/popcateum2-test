...
import (
	...
	"github.com/popcateum/go-popcateum/consensus/beacon"
	...
	"github.com/popcateum/go-popcateum/eth/fetcher"
...
// support all the operations needed by the Popcateum chain protocols.
...
// newHandler returns a handler for all Popcateum chain management protocol.
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	// Execute the Popcateum handshake
	var (
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("Popcateum handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("Popcateum peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("Popcateum peer registration failed", "err", err)
		return err
	}
...
func (h *handler) unregisterPeer(id string) {
	...
	peer := h.peers.peer(id)
	if peer == nil {
		logger.Error("Popcateum peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `eth` peer if it exists
	logger.Debug("Removing Popcateum peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("Popcateum peer removal failed", "err", err)
	}
}
...
func (h *handler) Stop() {
	...
	log.Info("Popcateum protocol stopped")
}
