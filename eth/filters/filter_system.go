...
// Package filters implements an popcateum filtering system for block,
...
type subscription struct {
	...
	typ       Type
	created   time.Time
	logsCrit  popcateum.FilterQuery
	logs      chan []*types.Log
...
func (es *EventSystem) SubscribeLogs(crit popcateum.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit popcateum.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit popcateum.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit popcateum.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
