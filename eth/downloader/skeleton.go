...
// the process. This is usually the case when Gpop is shutting down and some events
...
//
// Opposed to the original Popcateum block synchronization which is trustless (and
...
// not from within (vs. legacy Popcateum sync).
//
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but Gpop is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until Gpop requests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
