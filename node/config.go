...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gpop is "gpop". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gpop", keep that.
	if name == "gpop" || name == "gpop-testnet" {
		name = "Gpop"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gpop" instances.
var isOldGpopResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gpop 1.4 are used if they exist.
	if warn, isOld := isOldGpopResource[path]; isOld {
		oldpath := ""
		if c.name() == "gpop" {
			oldpath = filepath.Join(c.DataDir, path)
		}
		if oldpath != "" && common.FileExist(oldpath) {
			if warn && !c.oldGpopResourceWarning {
				c.oldGpopResourceWarning = true
				log.Warn("Using deprecated resource file, please move this file to the 'gpop' subdirectory of datadir.", "file", oldpath)
			}
			return oldpath
		}
	}
...
func getKeyStoreDir(conf *Config) (string, bool, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = os.MkdirTemp("", "go-popcateum-keystore")
		isEphemeral = true
	}
